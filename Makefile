...
MODPROBE_OUTPUT=$(shell lsmod | grep lunar)
...
LUNAR_CFLAGS=-DLUNAR_DRIVER_VERSION='\"$(DRIVER_VERSION)\"'

modules:
	@$(MAKE) EXTRA_CFLAGS="$(LUNAR_CFLAGS)" -C $(KERNEL_BUILD) M=$(CURDIR) $@
